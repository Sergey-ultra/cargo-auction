...
function LoadList() {
    ...
    const [isOpenLoadModal, setOpenLoadModal] = useState(false);
    const [currentLoadId, setCurrentLoadId] = useState(null);

    const openLoadBidModal = loadId => {
    ...
    return (
        <Fragment>
            <SendBidModal handleClose={handleCloseLoadModal} isOpen={isOpenLoadModal} currentLoadId={currentLoadId}/>
            <form  className="filter" id="filter">
                ...
                <div className="filter__row">
                    <Button>Сохранить как фильтр</Button>
            ...
            {list.length > 0
                ? <Fragment>
                    ...
                    <div className="table">
                        ...
                        {list.map(load => <LoadItem key={load.id} load={load} openLoadBidModal={openLoadBidModal}/>)}
                    </div>
